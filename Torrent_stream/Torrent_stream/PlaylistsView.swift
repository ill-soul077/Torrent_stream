...
@MainActor
final class PlaylistsVM: ObservableObject {
    ...
    func create(name: String, description: String) async {
        do {
            let playlist = try await APIService.shared.createPlaylist(name: name, description: description)
        ...
        NavigationView {
            ...
            .sheet(isPresented: $showCreate) {
                CreatePlaylistSheet { name, description in
                    await vm.create(name: name, description: description)
                }
            }
...
@MainActor
final class PlaylistDetailVM: ObservableObject {
    ...
    func removeItem(playlistId: String, itemId: String) async {
    ...
    var body: some View {
        NavigationView {
                ...
                if vm.isLoading {
                    ...
                    if detail.items.isEmpty {
                        ...
                        List {
                            ...
                            ForEach(Array(detail.items.enumerated()), id: \.element.id) { idx, item in
                                HStack(spacing: 12) {
                                    Text("\(idx + 1)")
                                        .font(.caption)
                                        .foregroundColor(.gray)
                                ...
                                .swipeActions(edge: .trailing) {
                                    Button(role: .destructive) {
                                        Task { await vm.removeItem(playlistId: detail.id, itemId: item.id) }
                                    } label: {
            ...
            .sheet(item: Binding(
                ...
                if let items = vm.detail?.items, value.index < items.count {
                    PlayerView(torrent: items[value.index].torrent.toTorrentItem())
                }
            }
        }
